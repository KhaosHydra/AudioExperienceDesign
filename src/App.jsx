...
// ox,oy = top-left of sprite in 512×384 room space; w,h = rendered size in px
// anchorFx/Fy = floor contact point as fraction of sprite w/h
// boxW/boxD = isometric floor-footprint in logical units (0-1); boxH = visual height above floor (px)
const S = {
  ...
  'Room - Bed.png':             {ox:189, oy:85,  w:170, h:128, anchorFx:0.4765, anchorFy:0.6328, boxW:0.30, boxD:0.22, boxH:65},
  'Room - Bookshelf.png':       {ox:13,  oy:105, w:138, h:158, anchorFx:0.4638, anchorFy:0.981,  boxW:0.24, boxD:0.09, boxH:90},
  'Room - Coffee.png':          {ox:231, oy:283, w:19,  h:25,  anchorFx:0.3684, anchorFy:0.96,   boxW:0.04, boxD:0.04, boxH:15},
  'Room - Cat.png':             {ox:252, oy:131, w:51,  h:30,  anchorFx:0.5294, anchorFy:1,      boxW:0.09, boxD:0.05, boxH:12},
  'Room - Duck teddy.png':      {ox:134, oy:189, w:31,  h:44,  anchorFx:0.5161, anchorFy:1,      boxW:0.06, boxD:0.05, boxH:22},
  'Room - Frog Teddy.png':      {ox:54,  oy:229, w:33,  h:45,  anchorFx:0.5152, anchorFy:0.9556, boxW:0.06, boxD:0.05, boxH:22},
  'Room - Plant oval base.png': {ox:136, oy:130, w:76,  h:68,  anchorFx:0.4605, anchorFy:1,      boxW:0.13, boxD:0.13, boxH:45},
  'Room - Plant.png':           {ox:128, oy:238, w:76,  h:89,  anchorFx:0.4737, anchorFy:0.9775, boxW:0.12, boxD:0.12, boxH:55},
  'Room - Speakers.png':        {ox:299, oy:159, w:57,  h:55,  anchorFx:0.4561, anchorFy:1,      boxW:0.10, boxD:0.08, boxH:28},
  'Room - Table.png':           {ox:208, oy:245, w:164, h:131, anchorFx:0.9878, anchorFy:1,      boxW:0.30, boxD:0.22, boxH:55},
  'Room - Vinyl Player.png':    {ox:302, oy:243, w:60,  h:43,  anchorFx:0.50,   anchorFy:0.95,   boxW:0.10, boxD:0.08, boxH:18},
...
// Returns true when two objects' isometric floor footprints overlap.
// Uses AABB check in logical coordinate space (lx/ly 0..1).
function isoFootprintCollides(lx1, ly1, sprite1, lx2, ly2, sprite2) {
  const s1=S[sprite1], s2=S[sprite2];
  if(!s1?.boxW || !s2?.boxW) return false;
  return Math.abs(lx1-lx2) < (s1.boxW+s2.boxW)/2 &&
         Math.abs(ly1-ly2) < (s1.boxD+s2.boxD)/2;
}
    ...
    const u=(ue)=>{
      if(trayDragRef.current&&roomRef.current){
        ...
        if(rx>=0&&rx<=r.width&&ry>=0&&ry<=r.height){
          ...
          const clx=clamp(lx,0.05,0.95), cly=clamp(ly,0.05,0.95);
          const collides=placed.some(o=>isoFootprintCollides(clx,cly,trayDragRef.current.sprite,o.lx,o.ly,o.sprite));
          if(!collides){
            setPlaced(prev=>[...prev,{...trayDragRef.current,lx:clx,ly:cly,iid:`${trayDragRef.current.id}_${Date.now()}`}]);
          }
        }
      }
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        ...
        <div ref={roomRef} onClick={()=>setSel(null)}
          ...
          {sorted.map(({o,i})=>{
            ...
            const showCollision=isDrag&&dragCollides;
            return(
              <img key={o.iid} src={sp(o.sprite)} alt={o.name} draggable={false}
                ...
                style={{position:'absolute',
                  ...
                  filter:showCollision
                    ?'drop-shadow(0 0 6px rgba(255,60,60,1)) drop-shadow(0 0 12px rgba(220,30,30,.8)) saturate(0.6) brightness(1.2)'
                    :iSel
